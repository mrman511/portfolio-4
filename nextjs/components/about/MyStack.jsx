  ...
  return (
    <section className={[styles.myStack, "flex w-full h-screen p-2 justify-center items-center"].join(' ')}>
      ...
      { currentCard !== 0 && <button 
        className={[styles.stackBtn, "absolute p-2 flex justify-center items-center -translate-y-1/2 top-1/2 -translate-x-1/2 left-8 sm:left-16 rounded-full z-20"].join(' ')}
        onClick={ ()=>{ clickPrev() } }
      >
      ...
      { currentCard !== (data.length - 1) && <button 
        className={[styles.stackBtn, "absolute p-2 flex justify-center items-center -translate-y-1/2 top-1/2 translate-x-1/2 right-8 sm:right-16 rounded-full z-20"].join(' ')}
        onClick={ ()=>{ clickNext() } }
      >
